...
judge() {
	...
	if [ "$_j_new" != "$_j_st" ]; then
		...
		if [ "$_j_new" = up ]; then
			...
			#
			# fw4 подставляет в зону wan КОНКРЕТНЫЕ устройства (wwan0 и т.п.) в
			# момент генерации набора правил, а не следит за ними позже. Штатно
			# это подхватывает hotplug (/etc/hotplug.d/iface/20-firewall на ifup),
			# но гонка остаётся даже с починенным +x на этом файле (см. Makefile
			# firewall4): у части модемов ПЕРВОЕ соединение после включения или
			# перезагрузки модуля идёт минутами (kernel_proto_prepare, дозвон), а
			# порой не срастается вовсе без ручного `firewall reload` - живой
			# случай владельца 23.08.2026. judge() уже подтвердил связность
			# настоящим пингом ЧЕРЕЗ УСТРОЙСТВО прямо перед этой строкой - надёжнее
			# сигнала «пора обновить fw4» на этом роутере нет. В фоне, чтобы не
			# задерживать круг проверки остальных линков; заходим сюда только на
			# СМЕНЕ состояния (down/gone/unknown -> up), не на каждом успехе.
